...
for templates in $(find logstash/templates/* -type f | awk 'BEGIN{FS="/"}{print $NF}' ); do docker exec logstash sh -c "curl -k -X PUT 'https://es01:9200/_template/$templates?pretty' -u 'elastic:$password' -H 'Content-Type: application/json' --form file=@/usr/share/logstash/templates/$templates"; done
echo
echo
echo "##########################################"
